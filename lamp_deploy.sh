...
sudo mysql -uroot -e "CREATE DATABASE Samuel;"
sudo mysql -uroot -e "CREATE USER 'yomi'@'localhost' IDENTIFIED BY '@samuel123';"
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON Samuel.* TO 'yomi'@'localhost';"
cd /var/www/laravel
...
sudo sed -i '25 s/=laravel/=Samuel/' /var/www/laravel/.env
sudo sed -i '26 s/=root/=yomi/' /var/www/laravel/.env
sudo sed -i '27 s/=/=@samuel123/' /var/www/laravel/.env
